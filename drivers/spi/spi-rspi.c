...
static int rspi_dma_slave_rx_start(struct spi_device *_tpSpi)
{
	...
	/* Configure slave signal to assert */
	tpRspi->spcmd |= SPCMD_SSLA(_tpSpi->cs_gpiod ? tpRspi->ctlr->unused_native_cs: _tpSpi->chip_select);
	/* CMOS output mode and MOSI signal from previous transfer */
	tpRspi->sppcr = 0;
	...
	if (!tpRspi->ucpRxBuf) {
		dev_err(&tpRspi->pdev->dev, "dma_alloc_coherent:%d",tpRspi->ulRxBufSize);		
		return -ENOMEM;
	}
