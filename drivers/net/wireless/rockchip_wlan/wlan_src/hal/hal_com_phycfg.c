...
int phy_load_tx_power_by_rate(_adapter *adapter, u8 chk_file)
{
	...
	if (chk_file
		...
		goto post_hdl;
			...
			RTW_ERR(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
			RTW_ERR(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
			RTW_ERR("%s doesn't exist, will use driver default power by rate parameter\n", PHY_FILE_PHY_REG_PG);
			RTW_ERR("Pls upload it into %s or the path defined by fw_path in GKI kernel\n", rtw_phy_file_path);
			RTW_ERR("<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<");
			RTW_ERR("<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<");
			rtw_warn_on(1);
	}
...
int phy_load_tx_power_limit(_adapter *adapter, u8 chk_file)
{
	...
	if (chk_file
		...
		RTW_ERR(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
		RTW_ERR(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
		RTW_ERR("%s doesn't exist, will use driver default power limit parameter !\n", PHY_FILE_TXPWR_LMT);
		RTW_ERR("Pls upload it into %s or the path defined by fw_path in GKI kernel\n", rtw_phy_file_path);
		RTW_ERR("<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<");
		RTW_ERR("<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<");
		rtw_warn_on(1);
